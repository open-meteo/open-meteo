        ...
        case .coordinates(let coordinates):
            return try coordinates.map { prepared in
                ...
                return (coordinates.locationId, timezone, time, domains.compactMap { domain in
                    return (domain, {
                        return try Self(domain: domain, lat: coordinates.latitude, lon: coordinates.longitude, elevation: coordinates.elevation, mode: params.cell_selection ?? .land, options: options)
                    })
                })
            }
                ...
                if dates.count == 0 {
                    ...
                    return gridpoionts.map( { gridpoint in
                        ...
                        return (locationId, timezone, time, [(domain, { () -> Self? in
                            guard let reader = try Self(domain: domain, gridpoint: gridpoint, options: options) else {
                                return nil
                            }
                            return reader
                        })])
