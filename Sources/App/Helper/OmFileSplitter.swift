...
struct OmFileSplitter {
    ...
    func updateFromTimeOrientedStreaming3D(variable: String, run: Timestamp, time: TimerangeDt, scalefactor: Float, compression: OmCompressionType = .pfor_delta2d_int16, onlyGeneratePreviousDays: Bool, onFileWritten: ((OmFileType) async -> Void)? = nil, supplyChunk: (_ y: Range<UInt64>, _ x: Range<UInt64>, _ member: Range<UInt64>) async throws -> ArraySlice<Float>) async throws {
        ...
        if nMembers > 1, let retainDays = domain.useRollingDays {
            ...
            return try await updateRollingTimeSeries(variable: variable, run: run, time: time, retainDays: retainDays, scalefactor: scalefactor, compression: compression, onFileWritten: onFileWritten, supplyChunk: supplyChunk)
        }
    ...
    /**
     ...
     `supplyChunk` should provide data for a couple of thousands locations at once. Updates are done as a stream to lower memory usage
     */
    func updateRollingTimeSeries(variable: String, run: Timestamp, time: TimerangeDt, retainDays: Int, scalefactor: Float, compression: OmCompressionType = .pfor_delta2d_int16, onFileWritten: ((OmFileType) async -> Void)? = nil, supplyChunk: (_ y: Range<UInt64>, _ x: Range<UInt64>, _ member: Range<UInt64>) async throws -> ArraySlice<Float>) async throws {
