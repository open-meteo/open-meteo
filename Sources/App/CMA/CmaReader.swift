...
struct CmaReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func get(raw: CmaSurfaceVariable, time: TimerangeDt) throws -> DataAndUnit {
        return try get(variable: .raw(.surface(raw)), time: time)
    }
    
    func prefetchData(derived: CmaVariableDerived, time: TimerangeDt) throws {
        switch derived {
    ...
    func get(derived: CmaVariableDerived, time: TimerangeDt) throws -> DataAndUnit {
        switch derived {
