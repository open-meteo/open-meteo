 ...
 */
public final class OmHttpReaderBackend: OmFileReaderBackend, Sendable {
        /// Size of remote http file
    public let count: Int
    
    public let eTag: String
    
    public let server: S3ServerHealth
    ...
    public init(context: S3ServerHealth, object: String) async throws {
        self.server = context
    ...
    public init(context: S3ServerHealth, object: String, count: Int, eTag: String, lastModified: Timestamp) {
        assert(eTag.hasPrefix("\""))
        assert(eTag.hasSuffix("\""))
        self.server = context
        self.object = object
    ...
    public func getData(offset: Int, count: Int) async throws -> ByteBuffer {
