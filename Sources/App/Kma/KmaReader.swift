...
struct KmaReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func get(raw: KmaVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        switch raw {
        case .surface(let variable):
            if reader.domain == .ldps && variable == .showers {
                /// Set showers to 0 for LDPS domain
                let showers = try await reader.get(variable: .surface(.precipitation), time: time).data.map({ max($0, 0) })
    ...
    func prefetchData(raw: KmaVariable, time: TimerangeDtAndSettings) async throws {
        switch raw {
        case .surface(let variable):
            if reader.domain == .ldps && variable == .showers {
                /// Set showers to 0 for LDPS domain
    ...
    func prefetchData(variable: KmaSurfaceVariable, time: TimerangeDtAndSettings) async throws {
        try await prefetchData(variable: .raw(.surface(variable)), time: time)
    }

    func get(raw: KmaSurfaceVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        return try await get(variable: .raw(.surface(raw)), time: time)
    }

    func prefetchData(derived: KmaVariableDerived, time: TimerangeDtAndSettings) async throws {
        switch derived {
        ...
        case .pressure(let v):
            ...
            case .windspeed, .wind_speed, .winddirection, .wind_direction:
                try await prefetchData(raw: .pressure(KmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time)
                try await prefetchData(raw: .pressure(KmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time)
            case .dewpoint, .dew_point, .relativehumidity:
                try await prefetchData(raw: .pressure(KmaPressureVariable(variable: .temperature, level: v.level)), time: time)
                try await prefetchData(raw: .pressure(KmaPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
            case .cloudcover, .cloud_cover:
                try await prefetchData(raw: .pressure(KmaPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
            }
        }
    ...
    func get(derived: KmaVariableDerived, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        switch derived {
        case .surface(let variableDerivedSurface):
            ...
            case .weathercode, .weather_code:
                let cloudcover = try await get(derived: .surface(.cloud_cover), time: time).data
        ...
        case .pressure(let v):
            ...
            case .windspeed, .wind_speed:
                let u = try await get(raw: .pressure(KmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time)
                let v = try await get(raw: .pressure(KmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time)
                let speed = zip(u.data, v.data).map(Meteorology.windspeed)
                return DataAndUnit(speed, u.unit)
            case .winddirection, .wind_direction:
                let u = try await get(raw: .pressure(KmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time).data
                let v = try await get(raw: .pressure(KmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time).data
                let direction = Meteorology.windirectionFast(u: u, v: v)
                return DataAndUnit(direction, .degreeDirection)
            case .dewpoint, .dew_point:
                let temperature = try await get(raw: .pressure(KmaPressureVariable(variable: .temperature, level: v.level)), time: time)
                let rh = try await get(raw: .pressure(KmaPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
                return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
            case .cloudcover, .cloud_cover:
            ...
            case .relativehumidity:
                return try await get(raw: .pressure(KmaPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
            }
        }
    }
}
