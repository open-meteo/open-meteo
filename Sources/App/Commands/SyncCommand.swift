...
struct SyncCommand: AsyncCommand {
    ...
    func syncModel(application: Application, concurrent: Int, server: String, model: DomainRegistry, variablesSig: [String], yearRange: Range<Timestamp>?, pastDays: Int, apikey: String?, downloadPressureNow: Bool) async throws {
        ...
        let remoteDirectories = try await S3List.s3list(context: .init(server: server, client: client, logger: logger), prefix: "data/\(model.rawValue)/", apikey: apikey, deadLineHours: 0.1).directories
        ...
        let toDownload: [S3List.ListV2File] = try await remoteDirectories.mapConcurrent(nConcurrent: concurrent) { remoteDirectory -> [S3List.ListV2File] in
            ...
            let remote = try await S3List.s3list(context: .init(server: server, client: client, logger: logger), prefix: remoteDirectory, apikey: apikey, deadLineHours: 0.1)
