...
Open-Meteo is an open-source weather API that offers free access for non-commercial use. No API key is required, so you can start using it immediately.

Explore the [API documentation](https://open-meteo.com/en/docs) or follow the [Open-Meteo blog](https://openmeteo.substack.com) for project updates.
...
Open-Meteo provides complete access to its source code and openly lists its data sources, with credit to the national weather services that produce them. You can launch your own weather API with Docker or the prebuilt Ubuntu package, inspect every stage of data processing, and adapt the service to your needs. Contributions are welcome.
...
- The [API documentation](https://open-meteo.com/en/docs) covers all public endpoints. Its source code and the API generator are maintained in the [Open-Meteo website repository](https://github.com/open-meteo/open-meteo-website).
- The free non-commercial API is hosted at [api.open-meteo.com](https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&hourly=temperature_2m), with GeoDNS routing requests to servers in Europe and North America. The API server source code is in this repository.
- The geocoding API source code is maintained in the [geocoding-api repository](https://github.com/open-meteo/geocoding-api).
- Larger changes are announced in the [Open-Meteo Blog](https://openmeteo.substack.com)
- The [Open-Meteo weather database](https://github.com/open-meteo/open-data) is redistributed through the AWS Open Data Sponsorship Program.
...
If you encounter a bug, please [open an issue](https://github.com/open-meteo/open-meteo/issues). For general ideas, questions, and community support, use [GitHub Discussions](https://github.com/open-meteo/open-meteo/discussions).

For other enquiries, contact info@open-meteo.com.
