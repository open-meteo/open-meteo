...
All three RRFS forecast-controller models also expose `precipitation_probability` from `ncep_rrfs_conus_ensemble`; it is an hourly ensemble statistic, not a separate probability calculated from the deterministic or 15-minute product. `ncep_rrfs_seamless` combines the underlying readers and does not have a separate stored variable catalog.
