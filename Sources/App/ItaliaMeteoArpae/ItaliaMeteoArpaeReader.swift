...
struct ItaliaMeteoArpaeReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func prefetchData(derived: ItaliaMeteoArpaeVariableDerived, time: TimerangeDtAndSettings) throws {
        ...
        case .pressure(let v):
            switch v.variable {
            case .windspeed:
                try prefetchData(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .wind_speed, level: v.level)), time: time)
            case .winddirection:
                try prefetchData(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .wind_direction, level: v.level)), time: time)
            case .dewpoint, .dew_point:
                try prefetchData(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .temperature, level: v.level)), time: time)
                try prefetchData(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .relative_humidity, level: v.level)), time: time)
    ...
    func get(derived: ItaliaMeteoArpaeVariableDerived, time: TimerangeDtAndSettings) throws -> DataAndUnit {
        ...
        case .pressure(let v):
            switch v.variable {
            case .windspeed:
                return try get(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .wind_speed, level: v.level)), time: time)
            case .winddirection:
                return try get(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .wind_direction, level: v.level)), time: time)
            case .dewpoint, .dew_point:
                let temperature = try get(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .temperature, level: v.level)), time: time)
                let rh = try get(raw: .pressure(ItaliaMeteoArpaePressureVariable(variable: .relative_humidity, level: v.level)), time: time)
                return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
            case .cloudcover, .cloud_cover:
