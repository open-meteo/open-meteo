...
struct SPA2 {
    func julian_day(year: Int, month: Int, day: Int, hour: Int, minute: Int, second: Double, delta_ut1 dut1: Double, timezone tz: Double) -> Double {
        var dayDecimal: Double
        var julianDay: Double
        var a: Double

        dayDecimal = Double(day) + (Double(hour) - tz + (Double(minute) + (second + dut1) / 60.0) / 60.0) / 24.0
        ...
        julianDay = floor(365.25 * Double(adjustedYear + 4716)) + floor(30.6001 * Double(adjustedMonth + 1)) + dayDecimal - 1524.5

        if julianDay > 2299160.0 {
            a = floor(Double(adjustedYear) / 100.0)
            julianDay += (2 - a + floor(a / 4.0))
        }
