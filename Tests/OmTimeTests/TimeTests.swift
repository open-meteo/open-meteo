...
@Suite struct OmFileIOTimeTests {
    ...
    @Test func timeIteration() {
        ...
        #expect(range.map(\.iso8601_YYYY_MM_dd_HH_mm) == expected)

        let daily = TimerangeDt(start: Timestamp(2022, 7, 1), nTime: 5, dtSeconds: 86400)
        let expectedDaily = ["2022-07-01", "2022-07-02", "2022-07-03", "2022-07-04", "2022-07-05"]
        #expect(daily.prettyString() == "2022-07-01 to 2022-07-05")
        #expect(daily.iso8601_YYYYMMdd == expectedDaily)
        #expect(daily.map(\.iso8601_YYYY_MM_dd) == expectedDaily)
    }
