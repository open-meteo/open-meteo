    ...
    func downloadEcmwf(application: Application, domain: EcmwfDomain, base: String, run: Timestamp, variables: [EcmwfVariable], concurrent: Int, maxForecastHour: Int?) async throws -> [GenericVariableHandle] {
        ...
        for hour in forecastHours {
            ...
            func calcRh(rh: EcmwfVariable, q:  EcmwfVariable, t: EcmwfVariable, member: Int, hpa: Float) async throws {
                ...
                let fn = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: data)
            ...
            func calcWindComponents(u: EcmwfVariable, v: EcmwfVariable, gph: EcmwfVariable, member: Int, hpa: Float) async throws {
                ...
                handles.append(GenericVariableHandle(
                    ...
                    time: timestamp,
                    member: member,
                    fn: try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: u.scalefactor, all: uData)
                ))
                handles.append(GenericVariableHandle(
                    variable: v,
                    time: timestamp,
                    member: member,
                    fn: try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: u.scalefactor, all: vData)
                ))
            }
            ...
            for member in 0..<domain.ensembleMembers {
                ...
                if let dewpoint = await inMemory.get(variable: .dew_point_2m, timestamp: timestamp, member: member)?.data,
                   let temperature = await inMemory.get(variable: .temperature_2m, timestamp: timestamp, member: member)?.data {
                    ...
                    let fn = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: rh)
                    handles.append(GenericVariableHandle(
                    ...
                    let fn = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: rh1000)
                    handles.append(GenericVariableHandle(
                        variable: EcmwfVariable.relative_humidity_2m,
                        time: timestamp,
                        member: member,
                ...
                let fnCloudCoverLow = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: cloudcoverLow)
                let fnCloudCoverMid = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: cloudcoverMid)
                let fnCloudCoverHigh = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: cloudcoverHigh)
                let cloudcover = Meteorology.cloudCoverTotal(low: cloudcoverLow, mid: cloudcoverMid, high: cloudcoverHigh)
                let fnCloudCover = try writer.writeTemporary(compressionType: .pfor_delta2d_16bit, scalefactor: 1, all: cloudcover)
