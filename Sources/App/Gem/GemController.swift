...
public struct GemController {
    func query(_ req: Request) throws -> EventLoopFuture<Response> {
        ...
        let result = ForecastapiResultSet(timeformat: params.timeformatOrDefault, results: try prepared.map { prepared in
            ...
            guard let reader = try GemMixer(domains: domains, lat: coordinates.latitude, lon: coordinates.longitude, elevation: coordinates.elevation, mode: params.cell_selection ?? .land) else {
...
extension GemMixer {
    ...
    func getDaily(variable: GemDailyWeatherVariable, params: ApiQueryParameter, time timeDaily: TimerangeDt) throws -> DataAndUnit {
        ...
        case .winddirection_10m_dominant:
            // vector addition
            let u = try get(variable: .windspeed_10m, time: time).data.sum(by: 24)
            let v = try get(variable: .winddirection_10m, time: time).data.sum(by: 24)
