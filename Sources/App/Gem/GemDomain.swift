...
enum GemDomain: String, GenericDomain, CaseIterable {
    ...
    var lastRun: Timestamp {
        ...
        case .gem_hrdps_continental:
            ...
            return t.add(-2 * 3600).floor(toNearest: 6 * 3600)
        case .gem_hrdps_west:
            // Delay of ~3 hours to init (experimental data)
            // every 12 hours
            return t.add(-3 * 3600).floor(toNearest: 12 * 3600)
        case .gem_global_ensemble:
            return t.add(-3 * 3600).floor(toNearest: 12 * 3600)
    ...
    func getUrl(run: Timestamp, hour: Int, gribName: String, server: String?) -> String {
        ...
        case .gem_hrdps_west:
            ...
            let alphaServer = server?.contains("dd.alpha") == true ? server! : "https://dd.alpha.weather.gc.ca/"
            return "\(alphaServer)model_hrdps/west/1km/grib2/\(run.hh)/\(h3)/CMC_hrdps_west_\(gribName)_rotated_latlon0.009x0.009_\(yyyymmddhh)_P\(h3)-00.grib2"
        case .gem_global_ensemble:
    ...
    var grid: any Gridable {
        ...
        case .gem_hrdps_west:
            ...
            // Grid parameters: coordinate of first grid point (relative to rotated grid): 5°S 23°W
            // TODO: Verify exact lat/lon bounds and rotation parameters from actual GRIB files
            return ProjectionGrid(nx: 1330, ny: 1180, latitude: 48.0...60.0, longitude: -140.0...(-110.0), projection: RotatedLatLonProjection(latitude: -5.0, longitude: -23.0))
        case .gem_global_ensemble:
