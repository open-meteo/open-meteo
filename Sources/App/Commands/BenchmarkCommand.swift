...
final class BenchmarkCommand: Command {
    ...
    func run(using context: CommandContext, signature: Signature) throws {
        ...
        try run.measure("Compression in memory, large chunks", 191) {
            try OmFileWriter(dim0: data.count / 1024, dim1: 1024, chunk0: 1024, chunk1: 1024).writeInMemory(compressionType: .pfor_delta2d_16bit, scalefactor: 20, all: data)
        }
        
        let compressed = try run.measure("Compression in memory, small chunks", 199) {
            try OmFileWriter(dim0: data.count / 1024, dim1: 1024, chunk0: 8, chunk1: 128).writeInMemory(compressionType: .pfor_delta2d_16bit, scalefactor: 20, all: data)
        }
        ...
        try run.measure("Compress to file, small chunks", 202) {
            try OmFileWriter(dim0: data.count / 1024, dim1: 1024, chunk0: 8, chunk1: 128).write(file: file, compressionType: .pfor_delta2d_16bit, scalefactor: 20, all: data, overwrite: true)
        }
