...
struct JmaQuery: Content, QueryWithStartEndDateTimeZone {
    let latitude: Float
    let longitude: Float
    let hourly: [JmaVariableCombined]?
    let daily: [JmaDailyWeatherVariable]?
...
enum JmaDailyWeatherVariable: String, Codable {
...
enum JmaVariableDerivedSurface: String, Codable, CaseIterable {
 ...
 */
struct JmaPressureVariableDerived: PressureVariableRespresentable {
...
typealias JmaReader = GenericReader<JmaDomain, JmaVariable>

typealias JmaMixer = GenericReaderMixerCached<JmaDomain, JmaVariable>
...
extension JmaMixer {
    ...
    func get(variable: JmaVariableCombined, time: TimerangeDt) throws -> DataAndUnit {
        switch variable {
    ...
    func prefetchData(variables: [JmaVariableCombined], time: TimerangeDt) throws {
        for variable in variables {
            switch variable {
            case .raw(let variable):
            ...
            case .derived(let variable):
                ...
                case .pressure(let v):
                    ...
                    case .windspeed:
                        fallthrough
                    case .winddirection:
                        try mixer.prefetchData(variable: .pressure(JmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time)
                        try mixer.prefetchData(variable: .pressure(JmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time)
                    case .dewpoint:
    ...
    func get(variable: JmaVariableDerived, time: TimerangeDt) throws -> DataAndUnit {
        switch variable {
        ...
        case .pressure(let v):
            switch v.variable {
            case .windspeed:
                let u = try get(variable: .pressure(JmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time)
                let v = try get(variable: .pressure(JmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time)
                let speed = zip(u.data,v.data).map(Meteorology.windspeed)
                return DataAndUnit(speed, u.unit)
            case .winddirection:
                let u = try get(variable: .pressure(JmaPressureVariable(variable: .wind_u_component, level: v.level)), time: time).data
                let v = try get(variable: .pressure(JmaPressureVariable(variable: .wind_v_component, level: v.level)), time: time).data
                let direction = Meteorology.windirectionFast(u: u, v: v)
                return DataAndUnit(direction, .degreeDirection)
            case .dewpoint:
