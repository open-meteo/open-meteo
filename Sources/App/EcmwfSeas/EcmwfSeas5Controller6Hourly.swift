//enum EcmwfSeasVariableSingleLevelDerived: String, RawRepresentableString, GenericVariableMixable {
...
//
//    typealias Variable = VariableOrDerived<EcmwfSeasVariableSingleLevel, EcmwfSeasVariableSingleLevelDerived>
//
...
//    }
//
//    func prefetchData(derived: EcmwfSeasVariableSingleLevelDerived, time: TimerangeDtAndSettings) async throws {
//        switch derived {
...
//            try await prefetchData(raw: .temperature_2m, time: time)
//            try await prefetchData(raw: .dew_point_2m, time: time)
//        case .global_tilted_irradiance, .global_tilted_irradiance_instant, .diffuse_radiation, .direct_radiation:
//            try await prefetchData(raw: .shortwave_radiation, time: time)
//        case .et0_fao_evapotranspiration:
//            try await prefetchData(raw: .shortwave_radiation, time: time)
//            try await prefetchData(raw: .temperature_2m, time: time)
...
//    }
//
//    func get(derived: EcmwfSeasVariableSingleLevelDerived, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
//        switch derived {
//        case .wind_speed_10m, .windspeed_10m:
