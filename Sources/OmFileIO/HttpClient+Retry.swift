...
public enum CurlErrorNonRetry: NonRetryError {
    case unauthorized
...
public struct ExponentialBackOff: Sendable {
    ...
    public init(factor: TimeAmount = .milliseconds(1000), maximum: TimeAmount = .seconds(30)) {
        self.factor = factor
        self.maximum = maximum
    }
    
    public func waitTime(attempt n: Int) -> TimeAmount {
    ...
    public func deadLine(attempt n: Int) -> Date {
        Date().addingTimeInterval(Double(waitTime(attempt: n).nanoseconds) / 1_000_000_000)
    }
    
    public func sleep(attempt n: Int) async throws {
