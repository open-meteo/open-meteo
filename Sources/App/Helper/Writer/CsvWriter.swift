    ...
    func toCsvResponse(concurrencySlot: Int?, withLocationHeader: Bool = true, logger: Logger) throws -> Response {
        let response = Response(body: .init(asyncStream: { writer in
            try await writer.submit(concurrencySlot: concurrencySlot, logger: logger) {
                ...
                for location in results {
                    try await location.weekly(variables: variables.weeklyVariables)?.writeCsv(into: &b, timeformat: timeformat, utc_offset_seconds: location.utc_offset_seconds, location_id: multiLocation ? location.locationId : nil)
                }
                for location in results {
...
extension ApiSectionSingle {
    fileprivate func writeCsv(into b: inout BufferAndAsyncWriter, timeformat: Timeformat, utc_offset_seconds: Int, location_id: Int?) async throws {
        ...
        for e in columns {
            if e.value.isFinite {
                b.buffer.writeString(",\(e.value.formatted(decimals: e.unit.apiSignificantDigits))")
            } else {
    ...
    fileprivate func writeCsv(into b: inout BufferAndAsyncWriter, timeformat: Timeformat, utc_offset_seconds: Int, location_id: Int?) async throws {
        ...
        for (i, time) in time.iterate(format: timeformat, utc_offset_seconds: utc_offset_seconds, quotedString: false, onlyDate: time.dtSeconds >= 86400).enumerated() {
            ...
            for e in columns {
                ...
                case .float(let a):
                    if a[i].isFinite {
                        b.buffer.writeString(",\(a[i].formatted(decimals: e.unit.apiSignificantDigits))")
                    } else {
                        b.buffer.writeString(",NaN")
                    }
