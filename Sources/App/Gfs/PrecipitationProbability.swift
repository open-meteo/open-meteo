...
enum ProbabilityReader {
    ...
    static func makeGfsReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReaderMixerSameDomain<GenericReader<GfsDomain, ProbabilityVariable>> {
    ...
    static func makeIconReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReaderMixerSameDomain<GenericReader<IconDomains, ProbabilityVariable>> {
        return await GenericReaderMixerSameDomain(reader: [
    ...
    static func makeIconGlobalReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<IconDomains, ProbabilityVariable> {
        guard let reader = try await GenericReader<IconDomains, ProbabilityVariable>(domain: .icon, lat: lat, lon: lon, elevation: elevation, mode: mode) else {
    ...
    static func makeNbmReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<NbmDomain, ProbabilityVariable>? {
        return try await GenericReader<NbmDomain, ProbabilityVariable>(domain: .nbm_conus, lat: lat, lon: lon, elevation: elevation, mode: mode)
    }

    /// Reader for probabilities based on MeteoFrance ARPEGE Europe 0.1°
    static func makeMeteoFranceEuropeReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<MeteoFranceDomain, ProbabilityVariable>? {
        return try await GenericReader<MeteoFranceDomain, ProbabilityVariable>(domain: .arpege_europe_probabilities, lat: lat, lon: lon, elevation: elevation, mode: mode)
    }

    /// Reader for probabilities based on ICON EU EPS
    static func makeIconEuReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<IconDomains, ProbabilityVariable>? {
        return try await GenericReader<IconDomains, ProbabilityVariable>(domain: .iconEu, lat: lat, lon: lon, elevation: elevation, mode: mode)
    }

    /// Reader for probabilities based on ICON D2 EPS
    static func makeIconD2Reader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<IconDomains, ProbabilityVariable>? {
        return try await GenericReader<IconDomains, ProbabilityVariable>(domain: .iconEu, lat: lat, lon: lon, elevation: elevation, mode: mode)
    }

    /// Reader for probabilities based on BOM ACCESS GLOBAL ENSEMBLE
    static func makeBomReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<BomDomain, ProbabilityVariable> {
        guard let reader = try await GenericReader<BomDomain, ProbabilityVariable>(domain: .access_global_ensemble, lat: lat, lon: lon, elevation: elevation, mode: mode) else {
    ...
    static func makeGemReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<GemDomain, ProbabilityVariable> {
        guard let reader = try await GenericReader<GemDomain, ProbabilityVariable>(domain: .gem_global_ensemble, lat: lat, lon: lon, elevation: elevation, mode: mode) else {
    ...
    static func makeEcmwfReader(lat: Float, lon: Float, elevation: Float, mode: GridSelectionMode) async throws -> GenericReader<EcmwfDomain, ProbabilityVariable> {
        guard let reader = try await GenericReader<EcmwfDomain, ProbabilityVariable>(domain: .ifs025_ensemble, lat: lat, lon: lon, elevation: elevation, mode: mode) else {
