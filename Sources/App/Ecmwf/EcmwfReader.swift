...
struct EcmwfReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func prefetchData(raw: EcmwfVariable, time: TimerangeDtAndSettings) async throws {
        try await reader.prefetchData(variable: raw, time: time)
    }

    func get(raw: EcmwfVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
    ...
    func get(derived: Derived, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        ...
        case .wind_speed_925hPa, .windspeed_925hPa:
            let v = try await get(raw: .wind_v_component_925hPa, time: time)
            let u = try await get(raw: .wind_u_component_925hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_850hPa, .windspeed_850hPa:
            let v = try await get(raw: .wind_v_component_850hPa, time: time)
            let u = try await get(raw: .wind_u_component_850hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_700hPa, .windspeed_700hPa:
            let v = try await get(raw: .wind_v_component_700hPa, time: time)
            let u = try await get(raw: .wind_u_component_700hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_600hPa, .windspeed_600hPa:
            let v = try await get(raw: .wind_v_component_600hPa, time: time)
            let u = try await get(raw: .wind_u_component_600hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_500hPa, .windspeed_500hPa:
            let v = try await get(raw: .wind_v_component_500hPa, time: time)
            let u = try await get(raw: .wind_u_component_500hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_400hPa, .windspeed_400hPa:
            let v = try await get(raw: .wind_v_component_400hPa, time: time)
            let u = try await get(raw: .wind_u_component_400hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_300hPa, .windspeed_300hPa:
            let v = try await get(raw: .wind_v_component_300hPa, time: time)
            let u = try await get(raw: .wind_u_component_300hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        case .wind_speed_250hPa, .windspeed_250hPa:
            let v = try await get(raw: .wind_v_component_250hPa, time: time)
            let u = try await get(raw: .wind_u_component_250hPa, time: time)
            let speed = zip(u.data, v.data).map(Meteorology.windspeed)
            return DataAndUnit(speed, .metrePerSecond)
        ...
        case .wind_speed_50hPa, .windspeed_50hPa:
            let v = try await get(raw: .wind_v_component_50hPa, time: time)
        ...
        case .cloud_cover_925hPa, .cloudcover_925hPa:
            let rh = try await get(raw: .relative_humidity_925hPa, time: time)
        ...
        case .cloud_cover_850hPa, .cloudcover_850hPa:
            let rh = try await get(raw: .relative_humidity_850hPa, time: time)
        ...
        case .cloud_cover_700hPa, .cloudcover_700hPa:
            let rh = try await get(raw: .relative_humidity_700hPa, time: time)
        ...
        case .cloud_cover_600hPa, .cloudcover_600hPa:
            let rh = try await get(raw: .relative_humidity_600hPa, time: time)
        ...
        case .cloud_cover_500hPa, .cloudcover_500hPa:
            let rh = try await get(raw: .relative_humidity_500hPa, time: time)
        ...
        case .cloud_cover_400hPa, .cloudcover_400hPa:
            let rh = try await get(raw: .relative_humidity_400hPa, time: time)
        ...
        case .cloud_cover_300hPa, .cloudcover_300hPa:
            let rh = try await get(raw: .relative_humidity_300hPa, time: time)
        ...
        case .cloud_cover_250hPa, .cloudcover_250hPa:
            let rh = try await get(raw: .relative_humidity_250hPa, time: time)
        ...
        case .relativehumidity_1000hPa:
            return try await get(raw: .relative_humidity_1000hPa, time: time)
        case .relativehumidity_925hPa:
            return try await get(raw: .relative_humidity_925hPa, time: time)
        case .relativehumidity_850hPa:
            return try await get(raw: .relative_humidity_850hPa, time: time)
        ...
        case .dew_point_925hPa, .dewpoint_925hPa:
            let temperature = try await get(raw: .temperature_925hPa, time: time)
            let rh = try await get(raw: .relative_humidity_925hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_850hPa, .dewpoint_850hPa:
            let temperature = try await get(raw: .temperature_850hPa, time: time)
            let rh = try await get(raw: .relative_humidity_850hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_700hPa, .dewpoint_700hPa:
            let temperature = try await get(raw: .temperature_700hPa, time: time)
            let rh = try await get(raw: .relative_humidity_700hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_600hPa, .dewpoint_600hPa:
            let temperature = try await get(raw: .temperature_600hPa, time: time)
            let rh = try await get(raw: .relative_humidity_600hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_500hPa, .dewpoint_500hPa:
            let temperature = try await get(raw: .temperature_500hPa, time: time)
            let rh = try await get(raw: .relative_humidity_500hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_400hPa, .dewpoint_400hPa:
            let temperature = try await get(raw: .temperature_400hPa, time: time)
            let rh = try await get(raw: .relative_humidity_400hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_300hPa, .dewpoint_300hPa:
            let temperature = try await get(raw: .temperature_300hPa, time: time)
            let rh = try await get(raw: .relative_humidity_300hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
        case .dew_point_250hPa, .dewpoint_250hPa:
            let temperature = try await get(raw: .temperature_250hPa, time: time)
            let rh = try await get(raw: .relative_humidity_250hPa, time: time)
            return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
    ...
    func prefetchData(derived: Derived, time: TimerangeDtAndSettings) async throws {
        ...
        case .wind_speed_1000hPa, .windspeed_1000hPa:
            try await prefetchData(raw: .wind_v_component_1000hPa, time: time)
            try await prefetchData(raw: .wind_u_component_1000hPa, time: time)
        case .wind_speed_925hPa, .windspeed_925hPa:
            try await prefetchData(raw: .wind_v_component_925hPa, time: time)
            try await prefetchData(raw: .wind_u_component_925hPa, time: time)
        case .wind_speed_850hPa, .windspeed_850hPa:
            try await prefetchData(raw: .wind_v_component_850hPa, time: time)
            try await prefetchData(raw: .wind_u_component_850hPa, time: time)
        case .wind_speed_700hPa, .windspeed_700hPa:
            try await prefetchData(raw: .wind_v_component_700hPa, time: time)
            try await prefetchData(raw: .wind_u_component_700hPa, time: time)
        case .wind_speed_600hPa, .windspeed_600hPa:
            try await prefetchData(raw: .wind_v_component_600hPa, time: time)
            try await prefetchData(raw: .wind_u_component_600hPa, time: time)
        case .wind_speed_500hPa, .windspeed_500hPa:
            try await prefetchData(raw: .wind_v_component_500hPa, time: time)
            try await prefetchData(raw: .wind_u_component_500hPa, time: time)
        case .wind_speed_400hPa, .windspeed_400hPa:
            try await prefetchData(raw: .wind_v_component_400hPa, time: time)
            try await prefetchData(raw: .wind_u_component_400hPa, time: time)
        case .wind_speed_300hPa, .windspeed_300hPa:
            try await prefetchData(raw: .wind_v_component_300hPa, time: time)
            try await prefetchData(raw: .wind_u_component_300hPa, time: time)
        case .wind_speed_250hPa, .windspeed_250hPa:
            try await prefetchData(raw: .wind_v_component_250hPa, time: time)
            try await prefetchData(raw: .wind_u_component_250hPa, time: time)
        case .wind_speed_200hPa, .windspeed_200hPa:
        ...
        case .cloud_cover_925hPa, .cloudcover_925hPa:
            try await prefetchData(raw: .relative_humidity_925hPa, time: time)
        case .cloud_cover_850hPa, .cloudcover_850hPa:
            try await prefetchData(raw: .relative_humidity_850hPa, time: time)
        case .cloud_cover_700hPa, .cloudcover_700hPa:
            try await prefetchData(raw: .relative_humidity_700hPa, time: time)
        case .cloud_cover_600hPa, .cloudcover_600hPa:
            try await prefetchData(raw: .relative_humidity_600hPa, time: time)
        case .cloud_cover_500hPa, .cloudcover_500hPa:
            try await prefetchData(raw: .relative_humidity_500hPa, time: time)
        case .cloud_cover_400hPa, .cloudcover_400hPa:
            try await prefetchData(raw: .relative_humidity_400hPa, time: time)
        case .cloud_cover_300hPa, .cloudcover_300hPa:
            try await prefetchData(raw: .relative_humidity_300hPa, time: time)
        case .cloud_cover_250hPa, .cloudcover_250hPa:
        ...
        case .dew_point_1000hPa, .dewpoint_1000hPa:
            ...
            try await prefetchData(raw: .relative_humidity_1000hPa, time: time)
        case .dew_point_925hPa, .dewpoint_925hPa:
            try await prefetchData(raw: .temperature_925hPa, time: time)
            try await prefetchData(raw: .relative_humidity_925hPa, time: time)
        case .dew_point_850hPa, .dewpoint_850hPa:
            try await prefetchData(raw: .temperature_850hPa, time: time)
            try await prefetchData(raw: .relative_humidity_850hPa, time: time)
        case .dew_point_700hPa, .dewpoint_700hPa:
            try await prefetchData(raw: .temperature_700hPa, time: time)
            try await prefetchData(raw: .relative_humidity_700hPa, time: time)
        case .dew_point_600hPa, .dewpoint_600hPa:
            try await prefetchData(raw: .temperature_600hPa, time: time)
            try await prefetchData(raw: .relative_humidity_600hPa, time: time)
        case .dew_point_500hPa, .dewpoint_500hPa:
            try await prefetchData(raw: .temperature_500hPa, time: time)
            try await prefetchData(raw: .relative_humidity_500hPa, time: time)
        case .dew_point_400hPa, .dewpoint_400hPa:
            try await prefetchData(raw: .temperature_400hPa, time: time)
            try await prefetchData(raw: .relative_humidity_400hPa, time: time)
        case .dew_point_300hPa, .dewpoint_300hPa:
            try await prefetchData(raw: .temperature_300hPa, time: time)
            try await prefetchData(raw: .relative_humidity_300hPa, time: time)
        case .dew_point_250hPa, .dewpoint_250hPa:
            try await prefetchData(raw: .temperature_250hPa, time: time)
            try await prefetchData(raw: .relative_humidity_250hPa, time: time)
        case .dew_point_200hPa, .dewpoint_200hPa:
        ...
        case .apparent_temperature:
            ...
            try await prefetchData(raw: .temperature_2m, time: time)
            try await prefetchData(derived: .windspeed_10m, time: time)
            try await prefetchData(raw: .shortwave_radiation, time: time)
        case .vapour_pressure_deficit, .vapor_pressure_deficit:
            try await prefetchData(derived: .relativehumidity_2m, time: time)
            try await prefetchData(raw: .temperature_2m, time: time)
