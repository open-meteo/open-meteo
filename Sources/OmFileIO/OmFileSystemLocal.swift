...
public enum OmFileSystemLocal {
    ...
    public actor Directory: Sendable, OmFileSystemDirectory {
        let fd: FileHandle?
        let inode: UInt64
        ...
        public init(fd: FileHandle, inode: UInt64) {
            OmFileSystemMetrics.fileLocalDirectoriesOpen.add(1, ordering: .relaxed)
            self.fd = fd
            self.inode = inode
            self.files = [:]
            self.directories = [:]
        ...
        public init(directories: [String: Directory]) {
            self.fd = nil
            self.inode = 0
        ...
        public func updateIfRequired(force: Bool = false) {
        ...
        public func updateRecursivelyIfRequired(now: Timestamp) async {
        ...
        private func forceUpdate() {
            ...
            OmFileSystemMetrics.fileLocalDirectoryUpdatedTotal.add(1, ordering: .relaxed)
            ...
            while let entry = readdir(dir) {
                ...
                if isDirectory {
                    ...
                    OmFileSystemMetrics.fileLocalDirectoryModifiedTotal.add(1, ordering: .relaxed)
                    do {
                    ...
                    OmFileSystemMetrics.fileLocalModifiedTotal.add(1, ordering: .relaxed)
                    do {
        ...
        public nonisolated func getDirectory(fullPath path: String) async -> Directory? {
        ...
        public nonisolated func getFile(fullPath path: String) async -> File? {
            assert(path.hasPrefix("/") == false)
    ...
    public actor File: OmFileSystemFile {
        nonisolated public let fd: FileHandle
        let inode: UInt64 // required to check while looping directories
        nonisolated public let size: Int64
        nonisolated public let modificationTimestamp: Timestamp
        ...
        init(fd: FileHandle, inode: UInt64, size: Int64, modificationTimestamp: Timestamp) {
            OmFileSystemMetrics.fileLocalOpen.add(1, ordering: .relaxed)
            self.fd = fd
            self.inode = inode
            self.size = size
        ...
        public func getPayload<T: OmLocalPayload>(ofType: T.Type) async throws -> T {
            switch payload {
            case .none:
