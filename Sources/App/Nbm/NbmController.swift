...
struct NbmReaderLowLevel: GenericReaderProtocol {
    ...
    func get(variable raw: NbmVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
...
struct NbmReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func get(derived: Derived, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        ...
        case .surface(let gfsVariableDerivedSurface):
            ...
            case .direct_radiation:
                let diffuse = try await get(derived: .surface(.diffuse_radiation), time: time)
            ...
            case .weather_code, .weathercode:
                ...
                let precipitation = try await get(raw: .surface(.precipitation), time: time).data
                let snowfall = try await get(raw: .surface(.snowfall), time: time).data
        ...
        case .pressure(let v):
            ...
            case .wind_speed, .windspeed:
                let u = try await get(raw: .pressure(NbmPressureVariable(variable: .wind_u_component, level: v.level)), time: time)
                let v = try await get(raw: .pressure(NbmPressureVariable(variable: .wind_v_component, level: v.level)), time: time)
                let speed = zip(u.data, v.data).map(Meteorology.windspeed)
                return DataAndUnit(speed, u.unit)
            case .wind_direction, .winddirection:
                let u = try await get(raw: .pressure(NbmPressureVariable(variable: .wind_u_component, level: v.level)), time: time).data
                let v = try await get(raw: .pressure(NbmPressureVariable(variable: .wind_v_component, level: v.level)), time: time).data
                let direction = Meteorology.windirectionFast(u: u, v: v)
                return DataAndUnit(direction, .degreeDirection)
            case .dew_point, .dewpoint:
                let temperature = try await get(raw: .pressure(NbmPressureVariable(variable: .temperature, level: v.level)), time: time)
                let rh = try await get(raw: .pressure(NbmPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
                return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
            case .cloudcover:
                return try await get(raw: .pressure(NbmPressureVariable(variable: .cloud_cover, level: v.level)), time: time)
            case .relativehumidity:
                return try await get(raw: .pressure(NbmPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
            }
        }
    }
}
