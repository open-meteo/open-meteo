...
struct UkmoReader: GenericReaderDerived, GenericReaderProtocol {
    ...
    func get(raw: UkmoVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        if case let .surface(variable) = raw {
            if domain == .global_deterministic_10km {
                ...
                case .showers, .snowfall_water_equivalent:
                    ...
                    let rain = try await get(raw: .rain, time: time).data
    ...
    func prefetchData(raw: UkmoVariable, time: TimerangeDtAndSettings) async throws {
        if case let .surface(variable) = raw {
            if domain == .global_deterministic_10km {
                // Global domain does not have amounts for showers and snowfall.
                // Precip and rain are available. The remainder must be either snow or showers.
    ...
    func prefetchData(variable: UkmoSurfaceVariable, time: TimerangeDtAndSettings) async throws {
        try await prefetchData(variable: .raw(.surface(variable)), time: time)
    }

    func get(raw: UkmoSurfaceVariable, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        return try await get(variable: .raw(.surface(raw)), time: time)
    }

    func prefetchData(derived: UkmoVariableDerived, time: TimerangeDtAndSettings) async throws {
        switch derived {
        ...
        case .pressure(let v):
            switch v.variable {
            case .windspeed:
                try await prefetchData(raw: .pressure(UkmoPressureVariable(variable: .wind_speed, level: v.level)), time: time)
            case .winddirection:
                try await prefetchData(raw: .pressure(UkmoPressureVariable(variable: .wind_direction, level: v.level)), time: time)
            case .dewpoint, .dew_point, .relativehumidity:
                try await prefetchData(raw: .pressure(UkmoPressureVariable(variable: .temperature, level: v.level)), time: time)
                try await prefetchData(raw: .pressure(UkmoPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
    ...
    func get(derived: UkmoVariableDerived, time: TimerangeDtAndSettings) async throws -> DataAndUnit {
        switch derived {
        case .surface(let variableDerivedSurface):
            ...
            case .showers:
                let precipitation = try await get(raw: .rain, time: time)
        ...
        case .pressure(let v):
            switch v.variable {
            case .windspeed:
                return try await get(raw: .pressure(UkmoPressureVariable(variable: .wind_speed, level: v.level)), time: time)
            case .winddirection:
                return try await get(raw: .pressure(UkmoPressureVariable(variable: .wind_direction, level: v.level)), time: time)
            case .dewpoint, .dew_point:
                let temperature = try await get(raw: .pressure(UkmoPressureVariable(variable: .temperature, level: v.level)), time: time)
                let rh = try await get(raw: .pressure(UkmoPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
                return DataAndUnit(zip(temperature.data, rh.data).map(Meteorology.dewpoint), temperature.unit)
            case .cloudcover, .cloud_cover:
            ...
            case .relativehumidity:
                return try await get(raw: .pressure(UkmoPressureVariable(variable: .relative_humidity, level: v.level)), time: time)
            }
        }
    }
}
