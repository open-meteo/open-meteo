...
struct OmFileSplitter {
     ...
     */
    func updateFromTimeOriented(variable: String, array2d: Array2DFastTime, time: TimerangeDt, skipFirst: Int, scalefactor: Float, compression: CompressionType = .p4nzdec256) throws {
        
        precondition(array2d.nTime == time.count)
        
        // Process at most 8 MB at once
        try updateFromTimeOrientedStreaming(variable: variable, time: time, skipFirst: skipFirst, scalefactor: scalefactor, compression: compression) { d0offset in
     ...
     */
    func updateFromTimeOrientedStreaming(variable: String, time: TimerangeDt, skipFirst: Int, scalefactor: Float, compression: CompressionType = .p4nzdec256, storePreviousForecast: Bool = false, supplyChunk: (_ dim0Offset: Int) throws -> ArraySlice<Float>) throws {
