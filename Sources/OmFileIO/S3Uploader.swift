 ...
 */
public enum S3Uploader {
    /// URL in form "https://S3-access-key:S3-secret-key@s3-host.tld/some-bucket/object"
    public static func upload<D: DataProtocol>(client: HTTPClient, data: D, url: String, contentType: String = "application/octet-stream", lastModified: Timestamp) async throws {
        var buffer = ByteBufferAllocator().buffer(capacity: data.count)
        buffer.writeData(data)
        try await upload(client: client, buffer: buffer, url: url, contentType: contentType, lastModified: lastModified)
    }
    
    public static func upload(client: HTTPClient, buffer: ByteBuffer, url: String, contentType: String = "application/octet-stream", lastModified: Timestamp) async throws {
    ...
    public static func uploadMultipart(client: HTTPClient, file: String, url: String, contentType: String = "application/octet-stream", lastModified: Timestamp, executor: LimitedConcurrencyExecutor) async throws -> S3MultiPartUploadPrepared {
    ...
    public static func uploadMultipart(client: HTTPClient, file: FilePath, url: String, contentType: String = "application/octet-stream", lastModified: Timestamp, executor: LimitedConcurrencyExecutor) async throws -> S3MultiPartUploadPrepared {
    ...
    /// URL in form "https://S3-access-key:S3-secret-key@s3-host.tld/some-bucket/object"
    public static func uploadMultipart<Data: S3UploadAble>(client: HTTPClient, data: Data, url: String, contentType: String = "application/octet-stream", lastModified: Timestamp, executor: LimitedConcurrencyExecutor) async throws -> S3MultiPartUploadPrepared {
        let logger = Logger(label: "S3Uploader")
    ...
    public static func uploadSync(client: HTTPClient, localDirectory: String, server: String, basePath: String, dryRun: Bool = false, exclude: [String] = [".*", "*~"]) async throws {
        let logger = Logger(label: "S3Uploader")
...
public struct S3MultiPartUploadPrepared: Sendable {
    let etags: [String]
    let url: String
    let encodedUploadId: String
    let lastModified: Timestamp

    public func abort(client: HTTPClient) async throws {
        let logger = Logger(label: "S3Uploader")
    ...
    public func commit(client: HTTPClient) async throws {
