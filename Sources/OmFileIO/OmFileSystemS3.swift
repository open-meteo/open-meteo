 ...
 */
public struct OmFileSystemS3: Sendable {
    let server: S3ServerHealth
    let root = Directory(prefix: "")
    
    public init(server: S3ServerHealth) {
        self.server = server
    }
    
    /// Make initial root directory access given a HTTP client and logger. Refreshes root if required
    public func getRoot(client: HTTPClient, logger: Logger) async throws -> DirectoryWithContext {
    ...
    public actor File {
        ...
        init(objectName: String, contentLength: Int, lastModified: Timestamp, eTag: String) {
            OmFileSystemMetrics.fileRemoteOpen.add(1, ordering: .relaxed)
        ...
        public func makeCachedClient(context: S3ServerHealth) -> OmHttpReaderBackend{
            return OmHttpReaderBackend(context: context, object: objectName, count: contentLength, eTag: eTag, lastModified: lastModified)
        }
        ...
        func updateFromDirectoryListing(context: S3ServerHealth, objectKey: String, contentLength: Int, lastModified: Timestamp, eTag: String) async {
            ...
            OmFileSystemMetrics.fileRemoteModifiedTotal.add(1, ordering: .relaxed)
            self.contentLength = contentLength
            self.lastModified = lastModified
            assert(eTag.hasPrefix("\""))
            assert(eTag.hasSuffix("\""))
        ...
        func getPayload<T: OmRemotePayload>(ofType: T.Type, context: S3ServerHealth, receivedFileModifiedError: Bool) async throws -> T? {
            ...
            case .initialising(let queue):
                OmFileSystemMetrics.fileRemotePayloadWaiting.add(1, ordering: .relaxed)
            ...
            case .updating(let old, let queue):
                ...
                if receivedFileModifiedError {
                    ...
                    OmFileSystemMetrics.fileRemotePayloadUpdateWaiting.add(1, ordering: .relaxed)
                    defer { OmFileSystemMetrics.fileRemotePayloadUpdateWaiting.add(-1, ordering: .relaxed) }
            ...
            case .ready(let payload):
                ...
                do {
                    ...
                    self.lastModified = newReader.lastModified
                    self.eTag = newReader.eTag
                    let new = try await payload.remoteUpdated(file: newReader)
    ...
    public struct DirectoryWithContext: OmFileSystemDirectory {
        public let directory: Directory
        let context: S3ServerHealth
        
        /// Find a directory for a path
        public func getDirectory(fullPath: String) async throws -> DirectoryWithContext? {
        ...
        public func getFile(fullPath: String) async throws -> FileWithContext? {
        ...
        public func getDirectory(name: String) async throws -> DirectoryWithContext? {
        ...
        public func getFile(name: String) async -> FileWithContext? {
    ...
    public actor Directory {
        ...
        init(prefix: String) {
            OmFileSystemMetrics.fileRemoteDirectoriesOpen.add(1, ordering: .relaxed)
            self.prefix = prefix
        }
            ...
            do {
                ...
                for directory in listed.directories {
                    ...
                    if directories[name] == nil {
                        OmFileSystemMetrics.fileRemoteDirectoryModifiedTotal.add(1, ordering: .relaxed)
        ...
        public func exportDirectories(directories: inout Set<String>, files: inout [String: (lastModified: Timestamp, size: Int64)]) async {
